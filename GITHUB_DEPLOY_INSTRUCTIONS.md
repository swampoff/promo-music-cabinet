...
## 🔄 Шаг 1.5: Синхронизация Figma Make ↔ Git

### Когда правки делаются в Figma Make
1. В Figma Make нажмите **Download** или **Export** (ZIP).
2. Распакуйте архив и замените файлы в репозитории.
3. Проверьте, что обновились `package.json`, `vite.config.ts` и папка `src/`.
4. Зафиксируйте изменения:
   ```bash
   git add .
   git commit -m "🎨 Figma Make: sync changes"
   git push origin main
   ```

### Когда правки делаются в Git
1. Сделайте коммит и отправьте изменения в GitHub.
2. В Figma Make обновите проект из Git (если подключён).
3. Если Git не подключён — скачайте ZIP из GitHub и загрузите его в Figma Make.
4. Обновите Preview в Figma Make (Refresh или перезапуск проекта).
