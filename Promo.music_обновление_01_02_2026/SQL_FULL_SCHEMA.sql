-- ============================================
-- PROMO.MUSIC - PROMOTION SYSTEM SQL SCHEMA
-- Version: 1.0
-- Date: 2026-01-27
-- Author: AI Assistant
-- ============================================
...
-- ============================================
-- TABLE 3: production_360_requests
-- Заявки на 360° продакшн
-- ============================================
...
-- ============================================
-- TABLE 5: media_outreach_requests
...
-- ============================================
-- TABLE 8: promotion_transactions
...
-- Политики для pitching_requests
CREATE POLICY IF NOT EXISTS "Artists can view own pitching requests"
  ON pitching_requests FOR SELECT
  USING (auth.uid()::text = artist_id);

CREATE POLICY IF NOT EXISTS "Artists can create pitching requests"
  ON pitching_requests FOR INSERT
  WITH CHECK (auth.uid()::text = artist_id);

CREATE POLICY IF NOT EXISTS "Artists can update own pitching requests"
  ON pitching_requests FOR UPDATE
  USING (auth.uid()::text = artist_id);

-- Аналогичные политики для остальных таблиц
CREATE POLICY IF NOT EXISTS "Artists can view own production requests"
  ON production_360_requests FOR SELECT
  USING (auth.uid()::text = artist_id);

CREATE POLICY IF NOT EXISTS "Artists can create production requests"
  ON production_360_requests FOR INSERT
  WITH CHECK (auth.uid()::text = artist_id);

CREATE POLICY IF NOT EXISTS "Artists can view own marketing campaigns"
  ON marketing_campaigns FOR SELECT
  USING (auth.uid()::text = artist_id);

CREATE POLICY IF NOT EXISTS "Artists can create marketing campaigns"
  ON marketing_campaigns FOR INSERT
  WITH CHECK (auth.uid()::text = artist_id);

CREATE POLICY IF NOT EXISTS "Artists can view own media requests"
  ON media_outreach_requests FOR SELECT
  USING (auth.uid()::text = artist_id);

CREATE POLICY IF NOT EXISTS "Artists can create media requests"
  ON media_outreach_requests FOR INSERT
  WITH CHECK (auth.uid()::text = artist_id);

CREATE POLICY IF NOT EXISTS "Artists can view own event requests"
  ON event_requests FOR SELECT
  USING (auth.uid()::text = artist_id);

CREATE POLICY IF NOT EXISTS "Artists can create event requests"
  ON event_requests FOR INSERT
  WITH CHECK (auth.uid()::text = artist_id);

CREATE POLICY IF NOT EXISTS "Artists can view own experiments"
  ON promo_lab_experiments FOR SELECT
  USING (auth.uid()::text = artist_id);

CREATE POLICY IF NOT EXISTS "Artists can create experiments"
  ON promo_lab_experiments FOR INSERT
  WITH CHECK (auth.uid()::text = artist_id);

CREATE POLICY IF NOT EXISTS "Artists can view own transactions"
