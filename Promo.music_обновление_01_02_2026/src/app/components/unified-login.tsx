...
import { Music2, Shield, ArrowLeft, Mail, Lock, Eye, EyeOff, Check } from 'lucide-react';
import { toast } from 'sonner';

type Role = 'artist' | 'admin' | null;

interface UnifiedLoginProps {
  onLoginSuccess: (role: 'artist' | 'admin') => void;
}
...
export function UnifiedLogin({ onLoginSuccess }: UnifiedLoginProps) {
  ...
  const handleRoleSelect = (role: 'artist' | 'admin') => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (validCredentials) {
        ...
        const userId = selectedRole === 'artist' ? 'artist_001' : 'admin_001';
        const userName = selectedRole === 'artist' ? 'Александр Иванов' : 'Администратор';
