...
export function SubscriptionProvider({ children, userId: providedUserId, initialSubscription }: SubscriptionProviderProps) {
  ...
  console.log('[SubscriptionProvider] Initializing with userId:', userId, 'subscription:', initialSubscription);
    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        if (isExpired && data.data.tier !== 'free') {
          console.warn('Subscription expired, reverting to free tier');
          setSubscription(DEFAULT_SUBSCRIPTION);
        } else {
  ...
  // Загрузка подписки при монтировании
  useEffect(() => {
    refreshSubscription();
  }, [userId]);

  // Автоматическая проверка истечения подписки каждые 5 минут
  useEffect(() => {
    const interval = setInterval(() => {
      if (subscription && subscription.tier !== 'free') {
        ...
        const now = new Date();
        if (expiresAt < now) {
          console.warn('Subscription expired during session');
          setSubscription(DEFAULT_SUBSCRIPTION);
        }
      }
    }, 5 * 60 * 1000); // 5 минут
...
export function useSubscription() {
  ...
  console.log('[useSubscription] Context value:', context);
  if (!context) {
