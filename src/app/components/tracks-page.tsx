...
export function TracksPage({ userCoins, onCoinsUpdate, onTrackClick }: TracksPageProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      >
        <div>
          <h1 className="text-5xl font-bold text-white mb-2">Мои треки</h1>
          <p className="text-gray-300 text-lg">Управляйте своей музыкой</p>
        </div>
        
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-yellow-500/20 to-orange-500/20 border border-yellow-400/30">
            <Coins className="w-5 h-5 text-yellow-400" />
            <span className="text-white font-semibold">{userCoins.toLocaleString()}</span>
            <span className="text-gray-400 text-sm">коинов</span>
          </div>
          ...
          <motion.button
            ...
            } text-white font-semibold transition-all duration-300 flex items-center gap-2`}
          >
            <Upload className="w-5 h-5" />
            {canUploadMore ? 'Загрузить трек' : `Лимит (${currentTrackCount}/${trackLimit})`}
          </motion.button>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        >
          <div className={`text-sm mb-2 ${canUploadMore ? 'text-gray-400' : 'text-orange-400'}`}>
        ...
        <motion.div
          ...
          className="p-5 rounded-2xl backdrop-blur-xl bg-green-500/10 border border-green-400/30 hover:bg-green-500/20 transition-all duration-300"
        >
          <div className="text-green-400 text-sm mb-2">Одобрено</div>
        ...
        <motion.div
          ...
          className="p-5 rounded-2xl backdrop-blur-xl bg-yellow-500/10 border border-yellow-400/30 hover:bg-yellow-500/20 transition-all duration-300"
        >
          <div className="text-yellow-400 text-sm mb-2">На модерации</div>
        ...
        <motion.div
          ...
          className="p-5 rounded-2xl backdrop-blur-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all duration-300"
        >
          <div className="text-gray-400 text-sm mb-2">Прослушивания</div>
        ...
        <motion.div
          ...
          className="p-5 rounded-2xl backdrop-blur-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all duration-300"
        >
          <div className="text-gray-400 text-sm mb-2">Лайки</div>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white focus:outline-none focus:border-cyan-400/50 transition-all duration-300 cursor-pointer"
          >
              ...
              >
                <div className="flex items-start gap-6">
                  ...
                  <div className="relative flex-shrink-0">
                    <div className="w-24 h-24 rounded-xl overflow-hidden ring-2 ring-white/10">
                      <ImageWithFallback
                        src={track.cover}
                        alt={track.title}
                    ...
                    {track.status === 'approved' && (
                      <motion.button
                        ...
                        className="absolute inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 rounded-xl"
                      >
                        {playingId === track.id ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-4 mb-3">
                      <div>
                        ...
                        <p className="text-gray-400 text-sm mb-2">{track.authors} • {track.genre}</p>
                        <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-lg ${statusConfig.bg} border ${statusConfig.border}`}>
                      ...
                      <div className="flex items-center gap-2">
                        {track.status === 'approved' && (
                          <motion.button
                            ...
                            className="px-4 py-2 rounded-xl bg-gradient-to-r from-purple-500 to-cyan-500 hover:from-purple-600 hover:to-cyan-600 text-white font-semibold transition-all duration-300 flex items-center gap-2 shadow-lg shadow-purple-500/15"
                          >
                        ...
                        {track.status === 'approved' && !track.isPaid && (
                          <motion.button
                            ...
                            className="px-4 py-2 rounded-xl bg-gradient-to-r from-yellow-500 to-orange-600 hover:from-yellow-600 hover:to-orange-700 text-white font-semibold transition-all duration-300 flex items-center gap-2 shadow-lg shadow-yellow-500/15"
                          >
                        ...
                        {track.status === 'approved' && track.isPaid && (
                          <div className="px-4 py-2 rounded-xl bg-green-500/20 border border-green-400/30 text-green-400 font-semibold flex items-center gap-2">
                    ...
                    {/* Description */}
                    {track.description && (
                      <p className="text-gray-300 text-sm mb-3 line-clamp-2">{track.description}</p>
                    )}
                    ...
                    {track.tags.length > 0 && (
                      <div className="flex flex-wrap gap-2 mb-3">
                        {track.tags.map((tag, i) => (
                          <span key={i} className="px-2 py-1 rounded-lg bg-cyan-500/10 text-cyan-400 text-xs font-semibold">
                            #{tag}
                          </span>
                        ))}
                      </div>
                    ...
                    {track.status === 'rejected' && track.rejectionReason && (
                      ...
                      >
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            <div className="text-red-400 font-semibold mb-1">Причина отклонения:</div>
                            <div className="text-gray-300 text-sm">{track.rejectionReason}</div>
                          </div>
                        </div>
                      </motion.div>
                    )}
