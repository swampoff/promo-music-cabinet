...
export function CoinsModal({ balance, onClose, onBalanceUpdate }: CoinsModalProps) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-2 sm:p-3 md:p-4"
      onClick={onClose}
    >
      <motion.div
      ...
      >
        ...
        <button
          ...
          className="absolute top-3 right-3 sm:top-4 sm:right-4 md:top-6 md:right-6 z-[60] w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 rounded-full backdrop-blur-xl bg-white/10 border border-white/20 hover:bg-white/20 transition-all duration-300 flex items-center justify-center"
        >
          ...
          <div className="p-3 sm:p-4 md:p-6 lg:p-8">
            ...
            <div className="text-center mb-4 sm:mb-6 md:mb-8">
              ...
              <p className="text-gray-400 text-xs sm:text-sm md:text-base">
                Текущий баланс: <span className="text-yellow-400 font-bold">{balance.toLocaleString()}</span> коинов
              </p>
            </div>
        ...
        <div className="relative z-10 border-t border-white/10 bg-slate-900/50 backdrop-blur-xl p-3 sm:p-4 md:p-6">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 mb-3 sm:mb-4">
            <div className="w-full sm:w-auto">
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-2xl sm:text-3xl font-bold text-white">{finalCoins.toLocaleString()}</span>
                ...
                {selectedPkg.bonusCoins > 0 && (
                  <span className="text-xs sm:text-sm text-emerald-400 font-semibold">
                    ({selectedPkg.baseCoins.toLocaleString()} + {selectedPkg.bonusCoins} бонус)
                  </span>
                )}
          ...
          <div className="mt-2 sm:mt-3 text-center text-[10px] sm:text-xs text-gray-400">
            Оплачивая, вы принимаете <span className="text-cyan-400 cursor-pointer hover:underline">условия использования</span> и <span className="text-cyan-400 cursor-pointer hover:underline">политику конфиденциальности</span>
          </div>
        </div>
      </motion.div>
