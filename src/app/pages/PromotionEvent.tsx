...
export function PromotionEvent() {
  ...
  const loadEvents = async (showToast = false) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-84730125/promotion/event/${userId}`,
        {
          headers: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-84730125/promotion/event/submit`,
        {
          method: 'POST',
          headers: {
