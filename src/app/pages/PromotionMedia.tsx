...
export function PromotionMedia() {
  ...
  const loadRequests = async (showToast = false) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-84730125/promotion/media/${userId}`,
        {
          headers: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-84730125/promotion/media/submit`,
        {
          method: 'POST',
          headers: {
