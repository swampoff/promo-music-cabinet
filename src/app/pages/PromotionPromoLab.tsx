...
export function PromotionPromoLab() {
  ...
  const loadExperiments = async (showToast = false) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-84730125/promotion/promolab/${userId}`,
        {
          headers: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-84730125/promotion/promolab/submit`,
        {
          method: 'POST',
          headers: {
