...
export function PromotionMarketing({ onBack }: PromotionMarketingProps) {
  ...
  const loadCampaigns = async (showToast = false) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-84730125/promotion/marketing/${userId}`,
        {
          headers: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-84730125/promotion/marketing/submit`,
        {
          method: 'POST',
          headers: {
