      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="fixed top-6 left-6 z-50 backdrop-blur-xl bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl px-4 py-2 flex items-center gap-2 text-white transition-all shadow-lg"
          >
            <ArrowLeft className="w-5 h-5" />
            <span className="hidden md:inline">Назад к услугам</span>
          </motion.button>
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900/20 to-gray-900 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
          ...
          {SERVICES.map((service, index) => {
            ...
            return (
              <motion.div
                ...
                className={`cursor-pointer backdrop-blur-xl bg-gradient-to-br ${service.gradient} border ${service.borderColor} rounded-2xl p-6 hover:shadow-2xl transition-all group`}
              >
                {/* Icon */}
                <div className={`w-14 h-14 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform ${service.iconColor}`}>
                ...
                <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/10">
                  <div>
                    {service.priceFrom > 0 ? (
                      ...
                      <span className="text-lg text-white/60">По запросу</span>
                    )}
                  </div>
                  <motion.div
                    whileHover={{ x: 5 }}
                    className={`w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center ${service.iconColor}`}
                  >
        ...
        <motion.div
          ...
          className="mt-8 backdrop-blur-xl bg-blue-500/10 border border-blue-500/30 rounded-2xl p-6"
        >
