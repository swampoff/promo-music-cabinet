...
export function PromotionProduction360() {
  ...
  const loadRequests = async (showToast = false) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-84730125/promotion/production360/${userId}`,
        {
          headers: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-84730125/promotion/production360/submit`,
        {
          method: 'POST',
          headers: {
